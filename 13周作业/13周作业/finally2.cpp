// finally1.cpp : Defines the entry point for the console application.
//
...
{
	int index;
	printf("piease enter index of students:\n");
	scanf("%d",&index);
	printf("no:");
...
void sort1(struct student s[])
{
	...
	printf("the sorted :");
	for(i=0;i<N;i++){
		printf("%s,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",s[i].name,s[i].number,s[i].grade[0],s[i].grade[1],s[i].grade[2],s[i].grade[3],s[i].grade[4],s[i].grade[5],s[i].grade[6],s[i].grade[7],s[i].grade[8],s[i].grade[9]);
	}
}
void sort2(struct student s[])
{
